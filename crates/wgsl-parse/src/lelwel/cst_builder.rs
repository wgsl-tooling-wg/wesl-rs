...
impl CstBuilder<'_> {
    ...
    fn start_rule(&mut self, rule: Rule) {
        // match rule {
    ...
    // fn start_node(&mut self, node: SyntaxKind) {
    //     self.builder.start_node(rowan::SyntaxKind::from(node));
    // }

    fn end_rule(&mut self, _node_ref: NodeRef, _rule: Rule) {
        // self.builder.finish_node();
    }

    fn token(&mut self, token: Token, index: CstIndex) {
        if matches!(
            token,
            Token::EOF
                | Token::EOFAttribute
